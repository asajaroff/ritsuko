...
def validate_environment():
    ...
    if features_unavailable:
        ...
        logging.warning(
            "Unavailable features (missing/invalid config): "
...
if authorized_users_env:
    ...
    authorized_users = [
        email.strip() for email in authorized_users_env.split(",") if email.strip()
    ]
    logging.info(
        f"Loaded {len(authorized_users)} authorized users from "
        "AUTHORIZED_USERS environment variable"
    )
else:
...
def send_message(message_dict):
    ...
    try:
        ...
        if result.get("result") != "success":
            logging.error(
                f'Failed to send message: {result.get("msg", "Unknown error")}'
            )
            return False
        return True
    except Exception as e:
...
def handle_message(message):
    ...
    try:
        ...
        bot_mentioned = (
            bot_user_id in mentioned_user_ids
            or "@**Ritsuko**" in content
            or "@_**Ritsuko**" in content
        )
        ...
        if msg_type == "stream":
            ...
            non_bot_users = [
                r for r in display_recipient if r["email"] != os.environ["ZULIP_EMAIL"]
            ]
        ...
        if message["sender_email"] not in authorized_users:
            ...
            try:
                ...
                from commands import handle_ai, parse_command
                ...
                if command == "ai":
                    response = handle_ai(
                        message, args, send_message_callback=send_message
                    )
                else:
                ...
                response = (
                    f"I encountered an error with the command format: {str(e)}\n\n"
                ...
                response = (
                    "Your request timed out. This usually means the external "
                ...
                response = (
                    "I couldn't connect to an external service required to "
                    "process your request.\n\n"
                    "Please try again later. If this persists, contact your "
                    "administrator."
                )
            except Exception as e:
                logging.error(
                    f"Unexpected error during command execution: {e}", exc_info=True
                )
                response = (
                ...
                send_message(
                    {
                        ...
                        "to": [
                            ...
                            if r["email"] != os.environ["ZULIP_EMAIL"]
                        ],
                        "content": response,
                    }
                )
        ...
        try:
            error_response = (
                "I encountered a critical error processing your message. "
                ...
                send_message(
                    {
                        ...
                        "to": [
                            r["email"]
                            for r in message.get("display_recipient", [])
                            if r.get("email") != os.environ.get("ZULIP_EMAIL")
                        ],
...
def initialize_zulip_client():
    ...
    for attempt in range(1, max_retries + 1):
        try:
            logging.info(
                f"Attempting to connect to Zulip "
            ...
            if profile.get("result") != "success":
                raise Exception(
                    f"Failed to get profile: {profile.get('msg', 'Unknown error')}"
                )

            logging.info(
                f"Successfully connected! Bot profile: {profile.get('full_name')} "
                f"(ID: {profile.get('user_id')})"
            )
            return client, profile
        except Exception as e:
            logging.error(
                f"Failed to initialize Zulip client "
...
def main():
    ...
    while True:
        try:
            ...
            failure_count += 1
            logging.error(
                f"Connection error in message loop "
                f"(failure {failure_count}/{max_consecutive_failures}): {e}"
            )

            if failure_count >= max_consecutive_failures:
                logging.critical(
                    "Max consecutive connection failures reached. "
                    "Exiting for pod restart."
                )
                sys.exit(1)
            ...
            try:
                ...
                logging.error(
                    f"Failed to reinitialize after unexpected error: {reinit_error}"
                )
