...
def get_nautobot_devices(node):
  ...
  try:
    ...
    if query_results.status == 404:
      ...
      logging.error(f"HTTP error {query_results.status} when querying device '{node}'")
        ...
        if device.get('rack') and device['rack'] is not None:
          ...
          try:
            ...
            if rack_query.status >= 400:
              logging.warning(f"Failed to retrieve rack {rack_id} (HTTP {rack_query.status})")
            ...
            logging.warning(f"Unexpected error querying rack {rack_id}: {e}")
...
""")
    ...
    return [f"**Error**: Request timed out while retrieving device `{node}` from Nautobot. The API may be slow or unavailable. Please try again."]
  except urllib3.exceptions.MaxRetryError as err:
    logging.error(f"Max retries exceeded when querying device '{node}': {err}")
    return [f"**Error**: Failed to connect to Nautobot API after multiple attempts while querying device `{node}`. Please check network connectivity or try again later."]
