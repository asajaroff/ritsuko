...
from validation import sanitize_for_logging, validate_node_name

sources = [{"vmall": "jsqMEvfSk&var"}]
...
def handle_node(message, nodes):
    ...
    for node in nodes:
        node = node
        url = (
            f"https://api.github.com/repos/{owner}/{repo}/contents//groups/{node}.json"
        )
        ...
        try:
            ...
            if response.status == 404:
                ...
                return (
                    f"**Error**: Node `{node}` not found in matchbox "
                    "repository.\n\n"
                    "Please verify the node name is correct. You can also try "
                    f"the `nautobot {node}` command to search for the node in "
                    "Nautobot."
                )
            elif response.status == 403:
                logging.error(
                    f"Access forbidden to matchbox repository for node '{node}'"
                )
                return (
                    f"**Error**: Access forbidden when querying node `{node}`. "
                    "The GitHub token may be invalid or lack necessary "
                    "permissions."
                )
            elif response.status >= 400:
                logging.error(
                    f"HTTP error {response.status} when querying node '{node}'"
                )
                return (
                    f"**Error**: Failed to retrieve node `{node}` from matchbox "
                    f"repository (HTTP {response.status})."
                )
            ...
            return (
                f"**Error**: Request timed out while retrieving node `{node}` "
                "from matchbox repository. GitHub API may be slow or "
                "unavailable. Please try again."
            )
        except urllib3.exceptions.MaxRetryError as e:
            logging.error(f"Max retries exceeded when querying node '{node}': {e}")
            return (
                f"**Error**: Failed to connect to GitHub API after multiple "
                f"attempts while querying node `{node}`. "
                "Please check network connectivity or try again later."
            )
        except json.JSONDecodeError as e:
            logging.error(f"Failed to parse matchbox data for node '{node}': {e}")
            return (
                f"**Error**: Failed to parse matchbox data for node `{node}`. "
                "The data may be corrupted or in an unexpected format."
            )
        except Exception as e:
            logging.error(f"Unexpected error when querying node '{node}': {e}")
            return (
                f"**Error**: An unexpected error occurred while retrieving "
                f"node `{node}`: {str(e)}"
            )

        return f"""### {node}
**Cluster**: {matchbox_data['metadata'].get('pod', 'N/A')} | **PublicIP**: \t\
{matchbox_data['metadata'].get('public_ip', 'N/A')}
...
## Grafana links
0. [Cluster Status Dashboard](https://graphs.eencloud.com/d/\
ceby874mq9ds0e/kubernetes-resource-requests-limits-by-node-vm?orgId=1&\
var-pod=bebxvkipvaio0e&var-node=All)
1. [Kubernetes node monitoring](https://graphs.eencloud.com/d/000000001/\
kubernetes-node-monitoring?orgId=1&var-Pod={vm_source}-Node={node})
2. [Node exporter detailed](https://graphs.eencloud.com/d/bovUFBfGz/\
node-exporter-detailed?orgId=1&var-Pod={vm_source}-Node={node}&\
var-job=kubernetes-service-endpoints&\
var-diskdevices=%5Ba-z%5D%2B%7Cnvme%5B0-9%5D%2Bn%5B0-9%5D%2B)
3. [Node monitoring -DC-](https://graphs.eencloud.com/d/aedj1sncwnpc0a/\
dc-node-monitoring?orgId=1&refresh=1m&var-eepod={vm_source}-\
...
def node_info(node_name):
    ...
    return (
        f"https://graphs.eencloud.com/d/bovUFBfGz/node-exporter-detailed?"
        f"orgId=1&var-Pod={vm_source}-Node={node_name}&"
        "var-job=kubernetes-service-endpoints&"
        "var-diskdevices=%5Ba-z%5D%2B%7Cnvme%5B0-9%5D%2Bn%5B0-9%5D%2B"
    )
