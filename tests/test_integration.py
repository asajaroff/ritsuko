...
SHARED_TOPIC = f'integration-tests-ritsuko-bot'
...
class BaseIntegrationTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.client = zulip.Client(
            email=ZULIP_TEST_EMAIL,
            api_key=ZULIP_TEST_API_KEY,
            site=ZULIP_SITE
        )
        ...
        try:
            ...
            bot_user = next(
                (u for u in users['members'] if u['email'] == ZULIP_BOT_EMAIL),
                None
            )
            if not bot_user:
    ...
    def send_private_message(self, content):
        ...
        self.assertEqual(result['result'], 'success', f'Failed to send message: {result}')
        return result
    ...
    def wait_for_bot_response(self, after_message_id, timeout=MESSAGE_TIMEOUT):
        ...
        request = {
            ...
            'num_after': 100,
            'narrow': [
                {'operator': 'sender', 'operand': ZULIP_BOT_EMAIL}
            ]
        }
...
class TestBotCommands(BaseIntegrationTest):
    ...
    def test_node_command_without_args(self):
        ...
        self.assertTrue(
            'node <node>' in response.lower() or 'node &lt;node&gt;' in response.lower(),
...
class TestBotMentionBehavior(BaseIntegrationTest):
    ...
    def test_bot_ignores_stream_without_mention(self):
        ...
        self.assertIsNone(response, 'Bot should not respond to messages without mention')
...
class TestBotResponseTimes(BaseIntegrationTest):
    ...
    def test_simple_command_response_time(self):
        ...
        self.assertIsNotNone(response)
        self.assertLess(response_time, 10, f'Response took {response_time:.2f}s, expected < 10s')
...
class TestNautobotCommand(BaseIntegrationTest):
    ...
    def test_nautobot_command_without_args(self):
        ...
        self.assertIn('usage', response.lower())
        self.assertTrue(
            'nautobot <node>' in response.lower() or 'nautobot &lt;node&gt;' in response.lower(),
...
def run_integration_tests():
    ...
    print('=' * 70)
    print(f'All stream tests will run in the same conversation: {RITSUKO_TEST_STREAM} > {SHARED_TOPIC}')
